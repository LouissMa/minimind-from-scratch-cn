...
![迷你预训练Loss曲线](results/pretrain_demo_loss.png)

> 说明：受限于 8GB 笔记本显卡，此为**迷你验证跑**（数据集前 12000 条、1 epoch / 750 步），
> 用于验证从零训练流程跑通、loss 正常下降。完整预训练（单卡预计 4~8 小时）待后续。
> 详见 [复现日志](docs/复现日志.md)。
